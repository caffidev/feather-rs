...
impl Solver {
    ...
    /// Registers a recipe with this `Solver`.
    /// Future calls to `solve()`
...
/// leftmost empty columns. Analogue to 2x2 crafting
...
pub fn translate(grid: &mut Grid, x: usize, y:usize) {
    ...
    for item in grid.iter_mut().skip(TABLE_WIDTH - x) {
        *item = [None, None, None]; // TODO: change according to size of crafting grid
    }
...
mod tests {
    ...
    fn solve_shaped() {
        ...
        let recipe = Recipe::Shaped(ShapedRecipe {
            ...
            output: ItemStack::new(Item::Stick, 4).unwrap(),
        });
        ...
        assert_eq!(
            ...
            Some(ItemStack::new(Item::Stick, 4).unwrap())
        );
    ...
    fn solve_shapeless() {
        ...
        let recipe = Recipe::Shapeless(ShapelessRecipe {
            ...
            output: ItemStack::new(Item::FlintAndSteel, 1).unwrap(),
        });
        solver.register(recipe);
        ...
        assert_eq!(
            solver.solve(&mut input),
            Some(ItemStack::new(Item::FlintAndSteel, 1).unwrap())
        );
